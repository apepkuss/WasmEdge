...
private:
  ...
  void unsafeInitVM();
  void unsafePreregisterHosts();
  void unsafePreregisterPlugins();
  ...
  /// Pre-registered module instances mapped to the configurations. For WASI.
  std::unordered_map<HostRegistration,
                     std::unique_ptr<Runtime::Instance::ModuleInstance>>
      HostRegModInst;
  /// Loaded and pre-registered module instances from plug-ins.
  std::vector<std::unique_ptr<Runtime::Instance::ModuleInstance>> PreRegModInst;
