...
use crate::{WasmEdgeResult, WasmValue};
use std::{
    cell::Cell,
    future::Future,
    io::Error,
...
pub struct AsyncWasmEdgeResult<'a, Stack, Output, P>
where
...
impl<'a, Stack, Output, P> AsyncWasmEdgeResult<'a, Stack, Output, P>
where
    Stack: stack::Stack + Send,
    P: FnMut() + Send,
{
    ...
    where
        F: FnOnce(AsyncYielder<'_, Output>) -> Output + 'a + Send,
    {
...
impl<'a, Stack, Output, P> Future for AsyncWasmEdgeResult<'a, Stack, Output, P>
where
...
impl<'a, Stack, Output, P> Drop for AsyncWasmEdgeResult<'a, Stack, Output, P>
where
    Stack: stack::Stack + Send,
    P: FnMut() + Send,
{
...
// pub type AsyncWasmEdgeResult<AsyncResult> =
//     Result<AsyncResult, wasmedge_types::error::WasmEdgeError>;

pub type WasmEdgeFuture<'a> =
    AsyncWasmEdgeResult<'a, EightMbStack, WasmEdgeResult<Vec<WasmValue>>, fn()>;
