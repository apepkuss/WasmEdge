        ...
        match ty {
            ExternalInstanceType::Func(_) => {
                ...
                match ctx_func_ty.is_null() {
                    ...
                    false => {
                        ...
                        let args_len = unsafe {
                            wasmedge::WasmEdge_FunctionTypeGetParametersLength(ctx_func_ty) as usize
                        };
                        ...
                        let returns_len = unsafe {
                            wasmedge::WasmEdge_FunctionTypeGetReturnsLength(ctx_func_ty) as usize
                        };
                        let mut returns = Vec::with_capacity(returns_len);
                        unsafe {
                            wasmedge::WasmEdge_FunctionTypeGetReturns(
                                ctx_func_ty,
                                returns.as_mut_ptr(),
                                returns_len as u32,
                            );
            ...
            ExternalInstanceType::Global(_) => {
                ...
                match ctx_global_ty.is_null() {
                    ...
                    false => {
                        // get the value type
                        let val = unsafe { wasmedge::WasmEdge_GlobalTypeGetValType(ctx_global_ty) };
                        let val_ty: ValType = val.into();

                        // get mutability
                        let val =
                            unsafe { wasmedge::WasmEdge_GlobalTypeGetMutability(ctx_global_ty) };
            ...
            ExternalInstanceType::Memory(_) => {
                ...
                match ctx_mem_ty.is_null() {
                    ...
                    false => {
                        let limit = unsafe { wasmedge::WasmEdge_MemoryTypeGetLimit(ctx_mem_ty) };
            ...
            ExternalInstanceType::Table(_) => {
                let ctx_tab_ty = unsafe {
                    wasmedge::WasmEdge_ImportTypeGetTableType(self.module.inner.0, self.inner.0)
                };
                match ctx_tab_ty.is_null() {
                    ...
                    false => {
                        ...
                        let elem_ty = unsafe { wasmedge::WasmEdge_TableTypeGetRefType(ctx_tab_ty) };
                        let elem_ty: RefType = elem_ty.into();

                        // get the limit
                        let limit = unsafe { wasmedge::WasmEdge_TableTypeGetLimit(ctx_tab_ty) };
                        let limit: std::ops::RangeInclusive<u32> = limit.into();
...
impl<'module> Export<'module> {
    ...
    pub fn ty(&self) -> WasmEdgeResult<ExternalInstanceType> {
        ...
        match ty {
            ...
            ExternalInstanceType::Table(_) => {
                let ctx_tab_ty = unsafe {
                    wasmedge::WasmEdge_ExportTypeGetTableType(self.module.inner.0, self.inner.0)
                };
                match ctx_tab_ty.is_null() {
